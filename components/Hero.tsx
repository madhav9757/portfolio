...
export default function Hero() {
  ...
  const socialIcons = [
    {
      icon: Github,
      href: "https://github.com/madhavsemwal",
      label: "GitHub",
      color: "text-gray-600 dark:text-gray-300 hover:text-purple-600 dark:hover:text-purple-400",
    },
    {
      icon: Linkedin,
      href: "https://linkedin.com/in/madhavsemwal",
      label: "LinkedIn",
      color: "text-gray-600 dark:text-gray-300 hover:text-blue-600 dark:hover:text-blue-400",
    },
    {
      icon: Mail,
      href: "mailto:madhavsemwalofficial@gmail.com",
      label: "Email",
      color: "text-gray-600 dark:text-gray-300 hover:text-green-600 dark:hover:text-green-400",
    },
  ];
  ...
  return (
    // Added ID for scroll-spy and increased padding
    <motion.div
      id="home"
      ...
      animate="visible"
    >
      {/* Animated Background Gradient (More subtle and blurred) */}
      ...
      <motion.h1
        ...
        className="mt-6 text-6xl md:text-8xl font-extrabold leading-none tracking-tighter" // Increased size and tracking
      >
        Hi, I'm
      ...
      <motion.div variants={itemVariants} className="mt-8">
        <p className="text-xl md:text-2xl text-muted-foreground font-light max-w-4xl mx-auto">
          A **Full-Stack Developer** & Generative AI Enthusiast — crafting{" "}
          {/* Using a single keyframe animation for better performance/coherence */}
          <motion.span
            className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-primary to-purple-500 inline-block"
          ...
          , modern, and{" "}
          <motion.span
            className="font-bold text-transparent bg-clip-text bg-gradient-to-r from-purple-500 to-blue-500 inline-block"
            animate={{ scale: [1, 1.03, 1] }}
      ...
      <motion.div variants={itemVariants} className="mt-12 flex flex-wrap justify-center gap-5">
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Button 
            asChild 
            size="lg" 
            className="group px-8 py-7 text-lg font-semibold shadow-2xl shadow-primary/50 transition-all duration-300 hover:shadow-primary/80" // Enhanced styling
          >
            <a href="#contact">
              Start a Project
        ...
        <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
          <Button 
            ...
            variant="outline" 
            size="lg" 
            className="px-8 py-7 text-lg font-medium border-2 hover:bg-secondary/50 transition-colors" // Enhanced outline button
          >
      ...
      {/* Social Icons (Slightly larger, better hover colors) */}
        ...
        {socialIcons.map((social, index) => {
          ...
          return (
            <motion.div
              ...
              whileHover={{ scale: 1.15, rotate: 5 }} // Added rotate on hover
              whileTap={{ scale: 0.9 }}
            >
      ...
      {/* Stats (More separation and visual polish) */}
      <motion.div variants={itemVariants} className="mt-12 grid grid-cols-3 gap-6 max-w-2xl mx-auto">
        {stats.map((stat, index) => (
          ...
          >
            <motion.h3 className="text-4xl md:text-5xl font-extrabold text-primary tracking-tighter">
              {stat.number}
            </motion.h3>
